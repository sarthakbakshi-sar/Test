#!/usr/bin/env python3
"""
Forex RORO Backtest V3 — FTMO $120k Challenge
==============================================
V3 improvements over V2:

  PAIR-TREND CONFIRMATION (new):
    Only enter in direction of 1H EMA50.
    Long only if price > EMA50_1H (uptrend confirmed).
    Short only if price < EMA50_1H (downtrend confirmed).
    This prevents fighting the actual market while the macro is still bullish.

  JPY CARRY GUARD (new):
    Skip JPY longs if USDJPY fell >2% in the last 5 trading days.
    Detects BoJ-style carry unwinds before they're catastrophic.

  SESSION SIMPLIFICATION:
    Removed Asian session (net -$15k for AUD/JPY in V2, no benefit elsewhere).
    All pairs: London open + NY overlap only.

  MACRO (unchanged from V2):
    Pair-specific 5-component macro score (oil/copper/TNX/VIX/SPX).
    Score can only be -10, -6, -2, +2, +6, +10 (sum of five ±1 signals).
    Entry requires |score| ≥ 6 (i.e., 4 of 5 components agree).

  RETAINED FROM V2:
    1H RSI overbought/oversold filter.
    Session VWAP zone (-1σ/+0.3σ longs, -0.3σ/+1.0σ shorts).
    4H + 1H + 15m EMA alignment.
    NFP avoidance (first Friday of month NY skip).
    High-VIX Wednesday NY skip.
    Score-scaled sizing: |score|≥8 → 1.25×.
    FTMO constraints: daily -$6k, absolute floor $108k.
"""
...
def add_indicators(df15, df1h):
    ...
    if not df1h.empty:
        ...
        df['ema50_1h']  = ema(c1h, 50).reindex(df.index, method='ffill')  # V3 trend filter
...
def main():
    ...
    print(f"\n{'='*88}")
    print(f"  FINAL COMPARISON V3 — Pair-Trend Filter + JPY Carry Guard + No Asian Session")
    print(f"{'='*88}")
