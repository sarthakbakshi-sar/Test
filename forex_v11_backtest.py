#!/usr/bin/env python3
"""
Forex RORO Backtest V11 — Regime-Adaptive Dual-Strategy
========================================================
V10 post-mortem: 4 bad months had two distinct failure modes:
  Failure A — OIL_WEAK (Sep/Nov/Dec 2025): oil fell below EMA50.
    CAD scores cluster near 0; oil component flips to -1 but SPX/VIX stay
    positive. CAD/JPY has no LONG edge (can't short carry pair + oil weak).
    Fix: skip CAD/JPY in OIL_WEAK; lower CAD/CHF SHORT threshold to -4.

  Failure B — VIX_SPIKE (Jan 2026): VIX > EMA20 while SPX held.
    High copper+SPX macro kept scoring AUD/JPY and CAD/JPY LONGS but
    intraday chop meant VWAP pullback entries got whipsawed → WR 13%.
    Fix: skip all carry pairs when VIX > EMA20; halve all sizes.

REGIME ENGINE (computed daily from macro, applied at trade entry):
  BULL      oil>E50 AND copper>E50 AND VIX<E20 AND SPX>E50
            Full system, normal thresholds, 1.0× size
  OIL_WEAK  oil<E50  (not classified as risk-off)
            Skip CAD/JPY; CAD/CHF short threshold -4; 0.75× size
  VIX_SPIKE VIX > EMA20 * 1.05
            Skip AUD/JPY, CAD/JPY; 0.50× all sizes
  NORMAL    Catch-all: full pairs, normal thresholds, 1.0× size
"""
...
def print_report(ptdf, final_eq, challenge_ok, pair_trades_dict):
    ...
    sep = '=' * 80
    print(f"\n{sep}")
    print(f"  PORTFOLIO V11 — REGIME-ADAPTIVE TREND (CAD/CHF AUD/JPY CAD/JPY) + ORB (GBP/JPY)")
    print(f"{sep}")
...
# ── MAIN ──────────────────────────────────────────────────────────────────────
def main():
    print("V11: Regime-Adaptive TREND + ORB (Opening Range Breakout)")
