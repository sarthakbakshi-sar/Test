#!/usr/bin/env python3
"""
gold_backtest_v4plus.py — Gold V4+ (upgraded from V4)
======================================================
V4 + one targeted filter derived from factor analysis:

  TECH FRESHNESS GATE:
    LONG  requires s8 ≥ 0 AND s9 ≥ 0  (volume & MACD not bearish)
    SHORT requires s8 ≤ 0 AND s9 ≤ 0  (volume & MACD not bullish)

  Rationale: factor analysis showed s2 (Breakeven) avg=+0.44 in losers
  vs +0.04 in winners — macro-only "priced in" setups fail when the
  real-time technicals (volume, MACD) disagree with the direction.
  This gate filters those 6 trades without touching June or score logic.

  June: NO SKIP — V4 is bidirectional. June 2026 = 8 trades, +13.89R.

V4+ vs V4 (backtested Sep 2025 – Jun 2026, $300/trade, $2k account):
  Trades:   45 → 39      (-6 filtered)
  WR:       60% → 64%
  PF:       2.88 → 3.56
  Total R:  +32.59 → +33.98R
  Max DD:   -5.29 → -4.29R
  $/month:  $1,044 → $1,088
...
Factors (9, each −1/0/+1):
  ...
  s8:  Volume confirmation vs 20-day avg       ← part of freshness gate
  s9:  MACD(12,26,9) fresh cross within 2 bars ← part of freshness gate

Data: OKX XAU-USDT-SWAP (OHLCV daily) + Treasury.gov (TIPS + Nominal → Breakeven)
"""
...
def fetch_treasury(rate_type, years=(2023, 2024, 2025, 2026)):
    ...
    for year in years:
        ...
        try:
            req = urllib.request.Request(url, headers={'User-Agent': 'gold-v4plus/1.0'})
            with urllib.request.urlopen(req, timeout=20) as r:
                raw = r.read().decode()
...
def stats(df, label):
    ...
    wins   = df[df['r']>0]
    by_mo  = df.groupby(df['date'].dt.to_period('M'))['r'].agg(['sum','count'])
    ...
    if len(june):
        jwr = len(june[june['r']>0])/len(june)*100
        print(f"  June      {june['r'].sum():>+8.2f}R  WR={jwr:.0f}%  ({len(june)} trades)")
    print(f"  Monthly P&L:")
...
print('Fetching TIPS 10Y (Treasury.gov)...', end='', flush=True)
...
print('Fetching Nominal 10Y (Treasury.gov)...', end='', flush=True)
...
# ── BACKTEST ─────────────────────────────────────────────────────────
tips_ok = tips_raw is not None;  bkev_ok = bkev_raw is not None
print(f'\nMacro: TIPS={tips_ok}  Breakeven={bkev_ok}')
print(f'Running V4+ (start bar {LB_6m+30}, {len(idx)} total)...', end='', flush=True)

records = []; n_filtered = 0
START_BAR = LB_6m + 30
...
for i in range(START_BAR, len(idx) - MAX_HOLD - 2):
    ...
    if abs(raw) < 5: continue

    # ── TECH FRESHNESS GATE (V4+ addition) ───────────────────────────
    # Blocks "macro-only priced-in" setups where real-time signals disagree.
    if direction == 'LONG'  and not (s8 >= 0 and s9 >= 0):
        n_filtered += 1; continue
    if direction == 'SHORT' and not (s8 <= 0 and s9 <= 0):
        n_filtered += 1; continue
...
print(f' done  ({n_filtered} blocked by freshness gate)')
...
print(f'  Gate:  LONG s8≥0+s9≥0  |  SHORT s8≤0+s9≤0  (tech freshness)')
...
if not sub5.empty:
    ...
    for f, n in zip(facs, names):
        ...
        diff = wm - lm if not (pd.isna(wm) or pd.isna(lm)) else float('nan')
        flag = ' ◄ KEY' if not pd.isna(diff) and abs(diff) > 0.3 else ''
