#!/usr/bin/env python3
"""
Forex RORO Backtest V10 — Dual-Strategy: VWAP Trend + London ORB
=================================================================
Key insight from research (arXiv 1411.2153, ForexTester):
  Opening Range Breakout (ORB) at London open on GBP/USD:
    Documented 74% WR, 2.512 profit factor, ~1 trade/day
  This is NOT trend-following — it is pure momentum at the highest-
  liquidity moment in FX markets. Completely different edge from RORO.

V10 ARCHITECTURE — Two complementary signals:
  Signal A (TREND): CAD/CHF + AUD/JPY + CAD/JPY
    RORO macro score ≥ 6 (4/5 components), VWAP pullback entry
    Session: London 07:30-10:00 + NY 14:00-17:00 UTC
    Risk: 2% / 1.5% / 1%

  Signal B (ORB): GBP/USD + GBP/JPY
    Pre-London range (06:30-07:30 UTC) breakout at session open
    Direction filter: DXY macro for GBP/USD, carry macro for GBP/JPY
    Session: London open only (07:30-09:00 UTC, tight window)
    Risk: 1.5% each (pure momentum, higher WR expected)
    GBP/JPY: LONG ONLY (carry trade)

TARGET: 1+ trade/day, p<0.05, 10-15% monthly ROI
"""
...
def generate_trend_trades(pair, df, macro_score, vix_series, usdjpy_series=None):
    ...
    while i < N - 1:
        ...
        elif macro <= SCORE_SHORT:    direction = -1
        else:                         i += 1; continue
        ...
        trades_out.append(dict(pair=pair, entry_dt=entry_dt, exit_dt=exit_dt,
                               ...
                               signal_type='trend', r_mult=r_mult, macro=macro, size_mult=size_mult))
        i = j + 1
...
def generate_orb_trades(pair, df, macro_score, vix_series, usdjpy_series=None):
    ...
    while i < N - 1:
        ...
        size_mult = get_size_mult(abs(macro)) if abs(macro) >= SCORE_LONG else 1.0
        entry_px  = price
        entry_dt  = row.name
        ...
        trades_out.append(dict(pair=pair, entry_dt=entry_dt, exit_dt=exit_dt,
                               ...
                               signal_type='orb', r_mult=r_mult, macro=macro, size_mult=size_mult))
        i = j + 1
...
def print_report(ptdf, final_eq, challenge_ok, pair_trades_dict):
    ...
    sep = '=' * 80
    print(f"\n{sep}")
    print(f"  PORTFOLIO V10 — TREND (CAD/CHF AUD/JPY CAD/JPY) + ORB (GBP/JPY)")
    print(f"{sep}")
...
# ── MAIN ──────────────────────────────────────────────────────────────────────
def main():
    print("V10: Dual-strategy — TREND + ORB (Opening Range Breakout)")
    print("  TREND: CAD/CHF 2%  AUD/JPY 1.5%  CAD/JPY 1%  (London+NY, score≥6, VWAP)")
    print("  ORB:   GBP/JPY 1%  (London open only, range break 06:30-07:30)")
    print()
