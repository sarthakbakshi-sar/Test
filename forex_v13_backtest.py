#!/usr/bin/env python3
"""
Forex RORO Backtest V13 — Fix MACD/RSI conflict in trend entries
=================================================================
V12 post-mortem: MACD histogram >0 and RSI <50 are in logical conflict for
pullback entries. When price pulls back (RSI drops below 50), MACD histogram
is still NEGATIVE from the pullback momentum. The histogram only crosses zero
AFTER the pullback is over — at which point RSI has often already recovered
above 50. Result: both conditions are rarely true simultaneously → only
5 trend trades per pair over 20 months.

Fix for TREND entries (single change from V12):
  MACD: histogram STILL required > 0 (strict V12 rule — momentum already
        positive, not just improving)
  RSI:  <55 longs (was <50), >45 shorts (was >50) — small buffer so we
        catch pullbacks that are healthy but haven't fully exhausted to 50

  Why this instead of "histogram improving": relaxing MACD to "improving"
  allows false bounce entries (CAD/JPY 20% WR in V13b). Keeping MACD strict
  ensures the pullback has genuinely bottomed before entry.

ORB entries: unchanged — MACD >0 and RSI >50 for long breakouts is correct
             (breakout momentum must already be established).

Everything else from V12 (regime engine, volume ≥120%, OBV, all V11 macro
...
def add_indicators(df15, df1h):
    ...
    df['macd_hist_prev'] = df['macd_hist'].shift(1)  # previous bar histogram
...
def generate_trend_trades(pair, df, macro_score, vix_series, usdjpy_series=None):
    ...
    while i < N - 1:
        ...
        # ── Technical entry confirmations (V13) ──────────────────────────────
        ...
        # 1. MACD histogram > 0 for longs / < 0 for shorts (strict — momentum
        #    must already be positive before entry, not just turning)
        if not pd.isna(macd_hist):
            if direction == 1  and macd_hist <= 0: i += 1; continue
            if direction == -1 and macd_hist >= 0: i += 1; continue

        # 2. RSI(15m): <55 longs (slight buffer vs V12's strict <50), >45 shorts
        if not pd.isna(rsi_15m):
            if direction == 1  and rsi_15m >= 55: i += 1; continue
            if direction == -1 and rsi_15m <= 45: i += 1; continue
...
def print_report(ptdf, final_eq, challenge_ok, pair_trades_dict):
    ...
    sep = '=' * 80
    print(f"\n{sep}")
    print(f"  PORTFOLIO V13 — MACD hist>0 + RSI 55/45 + Volume ≥120% + OBV")
    print(f"{sep}")
...
# ── MAIN ──────────────────────────────────────────────────────────────────────
def main():
    print("V13: MACD hist>0 (strict) + RSI 55/45 buffer  |  same ORB as V12")
    print("  TREND: CAD/CHF 2%  AUD/JPY 1.5%  CAD/JPY 1%  (MACD pos + RSI<55 + Vol + OBV)")
    print("  ORB:   GBP/JPY 1%  (breakout + volume ≥120% + MACD pos + RSI>50 + OBV)")
