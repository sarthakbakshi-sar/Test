...
def send_telegram(signals, now_utc, context=None):
    if not TG_TOKEN or not TG_CHAT_ID:
        print("  (Telegram skipped — TG_TOKEN / TG_CHAT_ID not set)")
        return
    ctx = context or {}
    lines = [f"\U0001f514 *{now_utc.strftime('%Y-%m-%d %H:%M UTC')}*\n"]
    ...
    if signals:
        for s in signals:
            ...
            note    = s.get('note','')
            lines += [
                f"{'\U0001f534' if side=='SHORT' else '\U0001f7e2'} *{side} {s['sym']}*",
                f"  Entry: `${price:.4f}`",
        ...
        lines += [
            ...
            f"BTC: {'\U0001f7e2 BULL' if btc_regime=='BULL' else '\U0001f534 BEAR'}  ${btc_price:,.0f}  EMA200 ${btc_ema200:,.0f}",
            f"Mode: {'LONG setups only' if btc_regime=='BULL' else 'SHORT setups only'}",
        ]
        if g_price:
            lines += [
                f"\nGold: ${g_price:,.1f}  RSI={g_rsi:.1f}  VWAP={g_vd*100:+.2f}%  {g_trend}",
            ]
        lines.append(f"\n_Scanning every 30 min — will alert on signal_")
    ...
    try:
        ...
        if r.json().get('ok'):
            label = f"{len(signals)} signal{'s' if len(signals)>1 else ''}" if signals else "no signal update"
...
def scan():
    ...
    if not signals:
        ...
        for s in signals:
            ...
            rsi_str = f"{s['rsi_prev']:.1f} → {s['rsi']:.1f}"
            vr_str  = f"   Vol: {s['vol_ratio']:.1f}x" if s.get('vol_ratio') else ""
            print(f"\n  {'\U0001f534' if side=='SHORT' else '\U0001f7e2'} {side} | {s['sym']}")
    ...
    send_telegram(signals, now_utc, context=ctx)
    return signals
