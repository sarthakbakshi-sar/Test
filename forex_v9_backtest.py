#!/usr/bin/env python3
"""
Forex RORO Backtest V9 — London/NY + GBP/JPY + PA (Pin Bar)
=============================================================
V8 yielded ~6 London+NY trades/month at avg 0.178R → ~5-6% monthly.
Need 12%+. Two levers: more pairs + PA entries.

CHANGES FROM V8:
  - Asian session REMOVED (user trades London + NY only)
  - GBP/JPY added: LONG ONLY, 1% risk, London+NY
    Macro: SPX risk-on + VIX calm + TNX carry + pair trend
  - PA pin bar signal: relaxed VWAP zone (±1.5σ) when clear rejection candle
    Bull pin = lower wick > 2× body, close in top 40% of range
    Bear pin = upper wick > 2× body, close in bottom 40% of range
  - Per-pair risk tiers: CAD/CHF 2%, AUD/JPY 1.5%, CAD/JPY 1%, GBP/JPY 1%

TARGET: 10-15 trades/month, 12%+ in active months
"""
...
# Per-pair risk tiers (statistical confidence)
RISK_TIER = {
    'CAD/CHF': 0.020,   # proven, p=0.019
    'AUD/JPY': 0.015,   # positive, not significant
    'CAD/JPY': 0.010,   # borderline
    'GBP/JPY': 0.010,   # new pair, conservative
}

PAIRS = ['AUD/JPY', 'AUD/CHF', 'AUD/NZD', 'AUD/USD',
         'CAD/CHF', 'CAD/JPY', 'CHF/JPY', 'GBP/JPY']

ACTIVE_PAIRS = ['CAD/CHF', 'AUD/JPY', 'CAD/JPY', 'GBP/JPY']

MIN_DATE = pd.Timestamp('2024-09-01', tz='UTC')

# V9: London + NY only (no Asian — user confirmed)
SESSIONS = {
...
PAIR_SESSIONS = {
    'AUD/JPY': ['london_open', 'ny_overlap'],
    'CAD/JPY': ['london_open', 'ny_overlap'],
    'GBP/JPY': ['london_open', 'ny_overlap'],
...
PAIR_ATR = {
    ...
    'GBP/JPY': (0.0005, 0.0200),   # GBP/JPY: ~50-200 pip daily range
}

JPY_PAIRS       = {'AUD/JPY', 'CAD/JPY', 'CHF/JPY', 'GBP/JPY'}
LONG_ONLY_PAIRS = {'AUD/JPY', 'CAD/JPY', 'CHF/JPY', 'GBP/JPY'}
...
def is_pin_bar(row, direction):
    """
    PA: Pin bar = clear rejection candle at VWAP level.
    Bull pin (direction=1): long lower wick, close in top 40% of bar.
    Bear pin (direction=-1): long upper wick, close in bottom 40%.
    """
    ...
    if direction == 1:
        # Hammer: lower wick > 2× body, lower wick dominates, close in top 40%
        return (lower_wick > 2 * max(body, rng * 0.05) and
                lower_wick > upper_wick * 1.5 and
                c > l + 0.60 * rng)
    else:
        # Shooting star: upper wick > 2× body, upper wick dominates, close in bottom 40%
        return (upper_wick > 2 * max(body, rng * 0.05) and
...
def build_macro(pair, macro):
    ...
    if pair == 'CAD/CHF':
        ...
        # GBP = risk currency, JPY = safe haven → same risk-on/carry framework
        s2 = safe(spx, n_ema=50)           # risk-on → GBP up, JPY down
        s3 = safe(vix, n_ema=20, inv=True) # VIX calm → JPY weak
        s4 = safe(tnx, compare_shift=10)   # rising yields → carry favourable
    elif pair == 'AUD/CHF':
        s2 = safe(cop, n_ema=50)
        s3 = safe(spx, n_ema=50)
        s4 = safe(vix, n_ema=20, inv=True)
...
def generate_pair_trades(pair, df, macro_score, vix_series, usdjpy_series=None):
    ...
    while i < N - 1:
        ...
        # Entry signal: standard VWAP zone OR PA pin bar at VWAP ±1.5σ
        dist = (price - vwap) / vstd
        standard_vwap = (
            (direction == 1  and -1.0 <= dist <= 0.3) or
            (direction == -1 and -0.3 <= dist <= 1.0)
        )
        pa_pin = (
            is_pin_bar(row, direction) and
            (direction == 1  and -1.5 <= dist <= 0.5) or
            (is_pin_bar(row, direction) and
             direction == -1 and -0.5 <= dist <= 1.5)
        )
        if not standard_vwap and not pa_pin:
            i += 1; continue

        signal_type = 'pin_bar' if (not standard_vwap and pa_pin) else 'vwap'
...
def print_report(ptdf, final_eq, challenge_ok, pair_trades_dict):
    ...
    sep = '=' * 80
    print(f"\n{sep}")
    print(f"  PORTFOLIO V9 — London+NY | GBP/JPY + PA Pin Bar | Per-pair risk tiers")
    print(f"{sep}")
    ...
    print(f"\n  By session:")
    for sess in ['london_open', 'ny_overlap']:
    ...
    print(f"\n  By signal type:")
    for stype in ['vwap', 'pin_bar']:
        sub = ptdf[ptdf['signal_type'] == stype]
        if sub.empty: continue
        wr  = (sub['r_mult'] > 0).mean()
        avg = sub['r_mult'].mean()
        pnl = sub['pnl'].sum()
        print(f"    {stype:<12} {len(sub):>4}tr  WR {wr*100:.0f}%  avg {avg:+.3f}R  ${pnl:>+11,.0f}")
    ...
    for _, mr in st['monthly'].iterrows():
        ...
        if pct >= 12: months_12pct += 1
        flag = ' ★12%+' if pct >= 12 else ''
        print(f"    {mr['month']}  {mr['n']:>3}tr  WR {mr['wr']*100:.0f}%  "
              f"${mr['pnl']:>+10,.0f}  ({pct:+.1f}%)  ${eq_run:>10,.0f} {tick}{flag}")
    print(f"  Positive months: {pos}/{len(st['monthly'])}  |  12%+ months: {months_12pct}/{len(st['monthly'])}")
    ...
    tpw = st['tpw']
    tpm = tpw * 4.33
    avg_risk_pct = sum(RISK_TIER.values()) / len(RISK_TIER)
    exp_r = st['wr'] * (TP1_MULT/SL_MULT*TP1_FRAC + TP2_MULT/SL_MULT*TP2_FRAC*0.5) - (1 - st['wr'])
    exp_pnl = tpm * exp_r * ACCOUNT * avg_risk_pct
    print(f"\n  30-day challenge projection:")
    print(f"    Trades/month  : ~{tpm:.0f}")
    print(f"    Avg R/trade   : {st['r_mult'].mean() if hasattr(st, '__getitem__') and 'r_mult' in st else exp_r:+.3f}R (at {st['wr']*100:.0f}% WR)")
...
# ── MAIN ──────────────────────────────────────────────────────────────────────
def main():
    print("V9: London+NY | GBP/JPY + PA Pin Bar | Per-pair risk tiers")
    print(f"    Score: ±{SCORE_LONG} (4/5 macro)  |  Sessions: London 07:30-10:00 + NY 14:00-17:00")
    print(f"    Pairs: CAD/CHF 2%  AUD/JPY 1.5%  CAD/JPY 1%  GBP/JPY 1%")
    print(f"    PA: Pin bar at VWAP ±1.5σ (standard zone: LONG -1.0 to +0.3σ)")
    print()

    print("Loading 15m data...")
    data_15m = {}
    for p in ACTIVE_PAIRS:
        data_15m[p] = load_15m(p)
    ...
    for pair in ACTIVE_PAIRS:
        ...
        if tdf.empty:
            ...
            sig_counts  = tdf['signal_type'].value_counts().to_dict()
            print(f"  [{pair}] {n} candidates  {tpw:.2f}/wk  WR={wr*100:.0f}%  "
                  f"avgR={avgr:+.3f}  {sess_counts}  {sig_counts}")
