#!/usr/bin/env python3
"""
Forex RORO Backtest V5 — FTMO $120k Challenge
==============================================
V5 improvement over V4: Long-only JPY carry pairs + faster EMA gate.
...
def build_macro(pair, macro):
    ...
    # V4: suppress signal when macro direction contradicts daily pair trend (s1).
    # Long signals require pair above daily EMA50; short signals require below.
    # Prevents counter-trend macro trades — the most common source of losses.
    aligned = (np.sign(score) == np.sign(s1))
...
def run_backtest(pair, df, macro_score, vix_series, usdjpy_series=None):
    ...
    while i < N - 1:
        ...
        # V5: JPY carry pairs are long-only (carry trade drift; shorting recovery loses)
...
def main():
    ...
    print(f"\n{'='*88}")
    print(f"  FINAL COMPARISON V5 — Daily-Trend Gate + Long-Only JPY Carry Pairs + JPY Guard")
    print(f"{'='*88}")
